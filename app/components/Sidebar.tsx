"use client";
import { Users, FolderKanban, Bot, Globe, FileText, DollarSign, Image, Truck, BarChart3, CheckSquare, Building2, LayoutDashboard, ChevronRight, Sparkles } from "lucide-react";
const modules = [
    ...
    <aside className="w-[220px] flex-shrink-0 bg-[#0D0D0D] border-r border-[#1E1E1E] h-screen sticky top-0 flex flex-col overflow-hidden">
      ...
      <nav className="flex-1 px-3 pb-4 overflow-y-auto space-y-0.5">
        {modules.map((mod) => {
          const Icon = mod.icon;
          return (
            <button key={mod.id} className={`sidebar-link w-full text-left ${active === mod.id ? "active" : ""}`} onClick={() => onSelect(mod.id)}>
              <Icon size={15} /><span className="flex-1">{mod.label}</span>
