...
import { Plus, Calendar, Paperclip, MessageSquare, AlertTriangle } from "lucide-react";
...
const kanbanCols = [
  ...
  {
    id: "procurement", label: "Procurement", color: "#3B82F6",
    tasks: [
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between fade-up fade-up-1">
        <div>
          <h2 className="font-display text-xl font-semibold text-[#E8E8E8]">Projects & Tasks</h2>
          <p className="text-sm text-[#666] mt-0.5">19 active projects · 47 open tasks</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex bg-[#111] border border-[#1E1E1E] rounded-lg p-0.5">
            {["kanban", "list"].map((v) => (
              <button key={v} onClick={() => setView(v as "kanban" | "list")}
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${view === v ? "bg-[#1E1E1E] text-[#E8E8E8]" : "text-[#666] hover:text-[#E8E8E8]"}`}>
          ...
          <div key={col.id} className="flex-shrink-0 w-[230px]">
            ...
            <div className="flex items-center justify-between mb-2.5 px-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-[#555] bg-[#1A1A1A] rounded-full px-1.5 py-0.5">{col.tasks.length}</span>
              </div>
              ...
              {col.tasks.map((task) => (
                <div key={task.id} className="bg-[#111] border border-[#1E1E1E] rounded-xl p-3 cursor-pointer hover:border-[#C9A84C33] transition-all hover:-translate-y-0.5">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="text-xs font-medium text-[#E8E8E8] leading-snug">{task.title}</div>
                    {task.priority === "high" && <AlertTriangle size={11} className="text-red-400 flex-shrink-0 mt-0.5" />}
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 text-[#555]">
                      {task.comments > 0 && (
                        <span className="flex items-center gap-0.5 text-[10px]"><MessageSquare size={10} />{task.comments}</span>
                      )}
                      {task.files > 0 && (
                        <span className="flex items-center gap-0.5 text-[10px]"><Paperclip size={10} />{task.files}</span>
                      )}
                  ...
                  <div className="flex items-center gap-1.5 mt-2.5 pt-2.5 border-t border-[#1A1A1A]">
                    <div className="w-5 h-5 rounded-full bg-[#C9A84C22] border border-[#C9A84C33] flex items-center justify-center">
                      <span className="text-[9px] text-[#C9A84C] font-semibold">{task.assignee[0]}</span>
                    </div>
                    <span className="text-[10px] text-[#666]">{task.assignee}</span>
                  </div>
                </div>
              ))}

              {/* Add card */}
              <button className="w-full border border-dashed border-[#1E1E1E] rounded-xl py-2.5 text-[11px] text-[#444] hover:border-[#C9A84C33] hover:text-[#666] transition-colors flex items-center justify-center gap-1">
